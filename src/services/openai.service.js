/**
 * Servicio de integración con OpenAI para procesamiento de texto
 */
...
class OpenAIService {
  ...
  async parseServiceText(text) {
    try {
      Logger.info('Iniciando procesamiento de texto con OpenAI', 'OpenAI');
      Logger.info(`Longitud del texto: ${text.length} caracteres`, 'OpenAI');
      ...
      Logger.info('Enviando solicitud a OpenAI...', 'OpenAI');
      ...
      Logger.info(`Respuesta recibida de OpenAI: ${responseContent}`, 'OpenAI');
      ...
      try {
        ...
        if (Array.isArray(extractedData.vehiculo)) {
          ...
          Logger.info(`Vehículo convertido de array a string: ${extractedData.vehiculo}`, 'OpenAI');
        }
        ...
        Logger.info('Procesamiento de texto completado', 'OpenAI');
        return extractedData;
      } catch (jsonError) {
        Logger.error(`Error al parsear JSON: ${jsonError.message}`, 'OpenAI');
        Logger.error(`Contenido que no se pudo parsear: ${jsonStr}`, 'OpenAI');
        throw new Error(`No se pudo parsear la respuesta JSON: ${jsonError.message}`);
      }
    } catch (error) {
      Logger.logError('Error al procesar el texto con OpenAI', error, 'OpenAI');
      throw error;
    }
  }
  ...
  formatDataToMessages(data) {
    ...
    Logger.info(`Generados ${messages.length} mensajes a partir de los datos`, 'OpenAI');
    return messages;
  }
}
